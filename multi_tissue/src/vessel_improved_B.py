#!/usr/bin/env python3
"""
vessel_improved_B.py — Continuous Linear Ramp + LR Warmup
Expert #2 recommendation: F_scale linearly 0.3→1.0, with micro LR warmups at 0.1 boundaries.
...
# ═══════════════════════════════════════════════════
# 4. Training — Continuous Ramp + LR Warmup
# ═══════════════════════════════════════════════════

def get_f_scale(ep, epochs):
    """Linear ramp from 0.3 to 1.0 over all epochs."""
    return 0.3 + 0.7 * min(ep / epochs, 1.0)

def get_lr_with_micro_warmup(ep, epochs, base_lr, f_scale_prev, f_scale_curr):
    """
    Cosine decay baseline + micro warmup when F_scale crosses 0.1 boundary.
    Micro warmup: 100 epochs, bump LR by 5% then decay back.
    """
    # Base cosine decay
    cos_lr = base_lr * 0.01 + 0.5 * base_lr * (1 + math.cos(math.pi * ep / epochs))
    
    # Check if we crossed a 0.1 boundary
    prev_bucket = int(f_scale_prev * 10)
    curr_bucket = int(f_scale_curr * 10)
    
    if curr_bucket > prev_bucket:
        # We just crossed a boundary — this will be handled by tracking
        return cos_lr, True
    return cos_lr, False

def train_vessel_ramp(seed, epochs=2000, lr=1e-3, hidden_dim=64, n_layers=6, device=None):
    tissue = "vessel"
    ...
    print(f"\n{'='*70}")
    print(f"  METHOD B: CONTINUOUS RAMP + LR WARMUP | Seed={seed}")
    print(f"  VESSEL: E={E:.0f} Pa | ν={nu} | F_scale: 0.3→1.0 linear")
    ...
    # Micro warmup tracking
    micro_warmup_start = -1000  # epoch when micro warmup started
    micro_warmup_duration = 100
    micro_warmup_boost = 0.05  # 5% LR boost
    ...
    for ep in range(1, epochs+1):
        # Compute F_scale: linear ramp 0.3 → 1.0
        f_scale = get_f_scale(ep, epochs)
        ...
        # Check for 0.1 boundary crossing → trigger micro warmup
        if int(f_scale * 10) > int(prev_f_scale * 10) and ep > warmup_init:
            micro_warmup_start = ep
            boundary = int(f_scale * 10) / 10.0
            print(f"  🔄 F_scale crossed {boundary:.1f} at ep {ep} → micro LR warmup")

        # Apply micro warmup if active
        if 0 <= (ep - micro_warmup_start) < micro_warmup_duration:
            progress = (ep - micro_warmup_start) / micro_warmup_duration
            # Triangle: ramp up for first half, ramp down for second half
            if progress < 0.5:
                boost = micro_warmup_boost * (progress / 0.5)
            else:
                boost = micro_warmup_boost * (1.0 - (progress - 0.5) / 0.5)
    ...
    results = {"tissue":tissue,"method":"continuous_ramp_B","seed":seed,
    ...
    print(f"\n  {'='*70}")
    print(f"  ✅ METHOD B (seed={seed}) COMPLETE")
    ...
    ckpt_dir = f"/root/results/vessel_improved_B/seed_{seed}"
...
def main():
    ...
    print("╔══════════════════════════════════════════════════════════════╗")
    print("║  Method B: Continuous Ramp + LR Warmup for Vessel          ║")
    print("║  F_scale: 0.3 → 1.0 linear | micro LR warmup at 0.1 steps ║")
    print("╚══════════════════════════════════════════════════════════════╝")
    if torch.cuda.is_available():
        print(f"  GPU: {torch.cuda.get_device_name(0)}")

    train_vessel_ramp(seed=args.seed, epochs=args.epochs, lr=args.lr)
