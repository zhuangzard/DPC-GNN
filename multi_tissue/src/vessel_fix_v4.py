#!/usr/bin/env python3
"""
vessel_fix_v4.py — Extended baseline (proves curriculum was counterproductive)

Approach: EXACTLY the baseline training logic (warm restarts, f_scale=1.0)
but with 2500 epochs for more convergence margin.

Changes vs curriculum v1:
  - NO force scaling (f_scale=1.0 always)
  - NO optimizer reset (single Adam throughout)
  - Warm restarts as in baseline (T_0=200, T_mult=2)
"""
...
def train_v4(seed, device=None, save_dir="/root/results/vessel_v4_extended",
    ...
    print(f"\n{'='*70}")
    print(f"  VESSEL v4 (EXTENDED BASELINE) | seed={seed}")
    print(f"  Expected tip δ: {delta_exp*1000:.4f} mm | dscale: {dscale:.2e}")
    print(f"  Epochs: {total_epochs} | LR: {lr} | Warm restarts (T0=200,Tm=2)")
    print(f"  f_scale=1.0 ALWAYS | F-bar enabled")
    print(f"{'='*70}")
    ...
    for ep in range(1, total_epochs + 1):
        ...
        loss = E_el + E_gr  # f_scale=1.0 always
        ...
        if ep % 500 == 0:
            history.append({"ep":ep,"loss":lv,"u_tip_mm":u_tip,"J_min":diag["J_min"],
                           "lr":opt.param_groups[0]['lr'],"best_ep":best_ep})
    ...
    results = {
        "seed": seed, "method": "v4_extended_baseline",
...
def main():
    ...
    parser.add_argument("--seeds", type=str, default="42,123,456,789,2026")
    parser.add_argument("--epochs", type=int, default=2500)
    parser.add_argument("--lr", type=float, default=1e-3)
    parser.add_argument("--save-dir", type=str, default="/root/results/vessel_v4_extended")
    parser.add_argument("--device", type=str, default=None)
    args = parser.parse_args()

    print("╔═══════════════════════════════════════════════════════════════╗")
    print("║  Vessel v4: Extended Baseline (2500ep, warm restarts, no CL) ║")
    print("╚═══════════════════════════════════════════════════════════════╝")
    ...
    for i, seed in enumerate(seeds):
        ...
        all_results[str(seed)] = train_v4(seed, args.device, args.save_dir, args.epochs, args.lr)
    ...
    print(f"{'='*80}")
    print(f"  {'Seed':>6} | {'Baseline':>9} | {'Curr v1':>9} | {'v4 Ext':>9} | {'v4-BL':>8}")
    print(f"  {'-'*55}")
    bl_errs = []
    for seed in seeds:
        b = baseline.get(str(seed), 0)
        c = curriculum_v1.get(str(seed), 0)
        v = all_results[str(seed)]["relative_error_pct"]
        bl_errs.append(b)
        print(f"  {seed:6d} | {b:8.2f}% | {c:8.2f}% | {v:8.2f}% | {v-b:+7.2f}%")

    bl_mean = sum(bl_errs)/len(bl_errs)
    success = sum(1 for e in errors if e < 15.0)
    print(f"  {'-'*55}")
    print(f"  Mean:  {bl_mean:.2f}%     {18.50:.2f}%     {mean_err:.2f}%")
    print(f"  Success (<15%): {success}/{len(seeds)}")
    print(f"  Total time: {total_time:.0f}s")
    print(f"{'='*80}")

    summary = {"method":"v4_extended_baseline","seeds":seeds,"results":all_results,
