#!/usr/bin/env python3
"""
3D FEM Ground Truth with F-bar method for volumetric locking prevention.
Compressible Neo-Hookean, Hex8 elements + F-bar, vectorized.
Newton-Raphson with load stepping.

F-bar: F_bar = (J0/J)^(1/3) * F, where J0 is from element centroid.
This prevents volumetric locking for nearly-incompressible materials.
"""
import numpy as np
...
# Shape function derivatives at element center (0,0,0)
dN_center = np.array([
...
def assemble_fbar(coords, elems, u_vec, mu, lam):
    """Assembly with F-bar method to prevent volumetric locking."""
    ne = elems.shape[0]
    nn = coords.shape[0]
    ndof = 3 * nn
    xe = coords[elems]
    ...
    # Compute F at element centroid for F-bar
    J0c = np.einsum('ai,eaj->eij', dN_center, xe)  # (ne, 3, 3)
    J0cinv = np.linalg.inv(J0c)
    dNdX_c = np.einsum('ai,eij->eaj', dN_center, J0cinv)  # (ne, 8, 3)
    F_c = np.eye(3)[None,:,:] + np.einsum('eai,eaj->eij', ue, dNdX_c)
    J_c = np.linalg.det(F_c)  # (ne,)
    detJ0c = np.linalg.det(J0c)  # (ne,)
    ...
    for g in range(8):
        ...
        # Standard F at Gauss point
        F_gp = np.eye(3)[None,:,:] + np.einsum('eai,eaj->eij', ue, dNdX)
        J_gp = np.linalg.det(F_gp)
        
        # F-bar modification: F_bar = (J_c / J_gp)^(1/3) * F_gp
        # This replaces the volumetric part with the centroid value
        scale = (J_c / np.maximum(J_gp, 1e-10)) ** (1.0/3.0)
        F = scale[:, None, None] * F_gp  # (ne, 3, 3)
        
        J = np.linalg.det(F)  # Should be close to J_c
        J_safe = np.maximum(J, 1e-10)
        lnJ = np.log(J_safe)
        Finv = np.linalg.inv(F)
        FinvT = Finv.transpose(0, 2, 1)
        
        # 1st Piola-Kirchhoff stress (w.r.t. F_bar)
        P = mu * (F - FinvT) + lam * lnJ[:, None, None] * FinvT
        
        # Internal force (use original dNdX, P is w.r.t. F_bar)
        PdN = np.einsum('eij,eaj->eai', P, dNdX) * detJ0[:, None, None]
        for a in range(8):
            fe_all[:, 3*a:3*a+3] += PdN[:, a, :]
        
        # Tangent stiffness - use standard tangent with F_bar
        # (approximate: ignore cross-terms from F-bar linearization for simplicity)
...
def solve_tissue(name, p, g_target):
    ...
    print(f"  FEM(F-bar) tip: {fem_tip:.4f} mm (conv={converged}, iters={total_iters}, {elapsed:.1f}s)", flush=True)
...
def main():
    ...
    for name, info in GNN_RESULTS.items():
        ...
        try:
            ...
            r["old_error_vs_eb_pct"] = round(abs(info["gnn_tip_mm"] - info["eb_tip_mm"]) / info["eb_tip_mm"] * 100, 2)
            r["new_error_vs_fem_pct"] = round(abs(info["gnn_tip_mm"] - r["fem_tip_mm"]) / r["fem_tip_mm"] * 100, 2)
            results[name] = r
        except Exception as e:
            import traceback; traceback.print_exc()
            results[name] = {"error": str(e)}
    
    with open("/root/results/fem_ground_truth_fbar.json", "w") as f:
        json.dump(results, f, indent=2)
    
    print(f"\n{'='*70}", flush=True)
    print("FINAL: FEM(F-bar) vs GNN vs EB", flush=True)
    print(f"{'Tissue':<12} {'GNN':>8} {'FEM':>8} {'EB':>8} {'Old%':>7} {'New%':>7} {'FEM/EB':>7}", flush=True)
    print("-"*60, flush=True)
    for n, r in results.items():
        if "error" in r:
            print(f"{n:<12} ERROR: {r['error']}", flush=True)
        else:
